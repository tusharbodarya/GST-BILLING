...
if(isset($_REQUEST['purchaseid'])){
	$id = $_REQUEST['purchaseid'];
	$pinvo = mysqli_query($conn,"SELECT * FROM tbl_purchaseorder WHERE id='".$id."'") or die("SQL Query 2 Failed.");
}else if(isset($_REQUEST['Salesstockreturnid'])){
	$id = $_REQUEST['Salesstockreturnid'];
	$pinvo = mysqli_query($conn,"SELECT * FROM tbl_Salesstockreturn WHERE id='".$id."'") or die("SQL Query 3 Failed.");
}else if(isset($_REQUEST['csrid'])){
	$id = $_REQUEST['csrid'];
	$pinvo = mysqli_query($conn,"SELECT * FROM tbl_Purchasestockreturn WHERE id='".$id."'") or die("SQL Query 3 Failed.");
}
...
if(isset($accountid)){
	$Sales = mysqli_query($conn,"SELECT * FROM tbl_accounts  WHERE id='".$accountid."' AND is_user='".$loguser."'")or die("SQL Query 4 Failed.");
	$srow = mysqli_fetch_assoc($Sales);
	$sname = $srow['name'];
	$sgstno = $srow['gstno'];
	$scity = $srow['city'];
	...
	<div style="border: 3px solid;margin: 50px; background-color: white;" >
		...
		<div style="display: flex;">
			...
			<div style="width: 27%;border-top: 2px solid;border-left: 2px solid;">
				<div style="display: flex;background-color: #f2f2f2;height: 30px;padding-bottom: 30px;" >
					<p style="text-align: left;width: 50%;">Grand Total</p><p style="text-align:right;width: 50%;"><?php echo number_format((float)$total, 2, '.', ''); ?></p>
				</div>
				<div>
					Note :
				</div>
